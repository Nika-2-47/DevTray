	...
	static async Task<int> Main(string[] args)
	{
		...
		while (true)
		{
			...
			else if (cmd == "help")
			{
				...
				Console.WriteLine("fountain - パーティクル噴水エフェクト(任意のキーで停止)");
				Console.WriteLine("exit - アプリ終了");
			}
			else if (cmd == "bouncingball")
			{
				var bouncingball = new BouncingBallEffect();
				bouncingball.Run();
			}
			...
			else if (cmd == "fountain")
			{
				var fountain = new ParticleFountainEffect();
				fountain.Run();
			}
			else if (cmd == "hacker")
			{
